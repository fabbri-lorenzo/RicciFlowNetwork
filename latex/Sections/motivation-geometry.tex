...
From multilinear algebra, it should be familiar that a \emph{tensor} of type $(p,q)$ is a multilinear map which maps $p$ elements of $T^*_p \M$ and $q$ elements of $T_p \M$ to a real number:
\begin{equation}
    T^{(p,q)} \colon 
    \underbrace{T^*_p \M \otimes \dots \otimes T^*_p \M}_\text{$p$ times}
    \otimes
    \underbrace{T_p \M \otimes \dots \otimes T_p \M}_\text{$q$ times} \longrightarrow \R.
\end{equation}

Picking a coordinate basis and its dual basis,
\begin{equation}
    T^{(p,q)} = \tensor{T}{^{\mu_1}^\dots^{\mu_p}_{\nu_1}_\dots_{\nu_q}} \frac{\de}{\de x^{\mu_1}} \dots \frac{\de}{\de x^{\mu_p}} \ud x^{\nu_1} \dots \ud x^{\nu_q}.
\end{equation}

Then, taking $q$-vectors $V_a = V_a^\mu \de_\mu$ and $p$ one-forms $\omega_a = \omega_{a\mu} \ud x^\mu$,the action of $T^{(p,q)}$ on them is given by
\begin{equation}
    T(\omega_1, \dots, \omega_p; V_1, \dots, V_q) = \tensor{T}{^{\mu_1}^\dots^{\mu_p}_{\nu_1}_\dots_{\nu_q}} \omega_{1\mu_1} \dots \omega_{p\mu_p} V_1^{\nu_1} \dots V_q^{\nu_q}
\end{equation}
